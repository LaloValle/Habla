...
const RowPhoneme = ({data, index, activeIndex, columns, onClickHandler=()=>'', activeStyle='', hoverStyles=[], gridCols=''}) => {
    ...
    return (
        <div className={`col-span-full w-full grid ${gridCols ? gridCols : `grid-cols-${columns.length}` }  ${index%2 == 0 ? 'bg-gray-300' : 'bg-gray-100'} ${index === activeIndex ? activeStyle : hoverStyles ? toStyles(hoverStyles, 'hover:') : ''} ${activeIndex >= 0 ? 'cursor-pointer' : ''}`} onClick={()=>onClickHandler(index)}>
            {columns.map((col) => (
